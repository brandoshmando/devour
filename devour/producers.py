...
class BaseProducer(object):
    ...
    def produce(self, event=None, source=None, extras={}, context={}):
        if event:
            extras['event'] = event
        if source:
            extras['source'] = source

        topic = self.get_topic(event, source, context)
        partition_key = self.get_partition_key(event, source, context)
        schema_class = self.get_schema(event, source, context)
        message = self.get_message(extras, schema_class)
    ...
    def get_topic(self, event, source, context):
        """
...
class Producer(BaseProducer):
    ...
    def get_schema(self, event, source, context):
        schema_class = super(Producer, self).get_schema(event, source, context)
        return schema_class or schemas.Schema
